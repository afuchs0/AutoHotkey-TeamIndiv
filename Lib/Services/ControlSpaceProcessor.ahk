    ...
    HandleFieldSelection(tableName, result) {
        ...
        try {
            ...
            for index, rowData in selectedRows {
                ...
                ; WICHTIG: Versteckte Zeilenumbrüche (`r, `n) und Leerzeichen aus den Strings filtern!
                cleanTableName := Trim(tableName, " `t`r`n")
                cleanFieldName := Trim(fieldName, " `t`r`n")
                
                ; Tabellenname + Feldname (z.B. KUN_NAME)
                fullField := StrUpper(cleanTableName "_" cleanFieldName)
